...
int main(int argc, char *argv[])
{
    if (argc != 4)
    {
        puts("\033[31m./имя_программы <количество блоков> <количество потоков> <имя файла>\033[0m");
        return 1;
    }
    blocks = atoi(argv[1]);
    threads = atoi(argv[2]);
    ...
    {
        puts("blocks");
        puts("\033[31mКоличество блоков не должно быть больше 256\033[0m");
        free(filename);
        return 0;
    }
    if (status == -4)
    {
        puts("threads");
        puts("\033[31mКоличество потоков должно лежать в пределах от 4 до 64 и быть не больше, чем количество блоков\033[0m");
        free(filename);
        return 0;
    }
